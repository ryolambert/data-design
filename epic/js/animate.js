(function() {
	...
	function shiftPoint(p) {
		TweenLite.to(p, 1+1*Math.random(), {
			x:p.originX-100+Math.random()*100,
	...
	function drawLines(p) {
		...
		for(var i in p.closest) {
			...
			ctx.strokeStyle = 'rgba(80,215,39,'+ p.active+')';
			ctx.stroke();
		}
	}
	...
	function Circle(pos,rad,color) {
		...
		this.draw = function() {
			...
			ctx.fillStyle = 'rgba(80,215,39,'+ _this.active+')';
			ctx.fill();
		};
	}
	...
	function getDistance(p1, p2) {
		return 2*Math.pow(p1.x - p2.x, 2) + Math.pow(p1.y - p2.y, 2);
	}
